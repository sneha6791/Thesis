...
def update_clusters(RMcluster,agent):
    ...
    if agent==1:
        window_counter1 = window_counter1 + 1
        ...
        curr1 = RMcluster[min_rmsum_cluster1]
        if prev1>=curr1:
            ctr1 = ctr1+1
        print 'a1','\t',it,'\t',prev1,'\t',curr1,'\t',flag1,'\t',window_counter1,'\t',ctr1,'\t',min_rmsum_cluster1,'\t',RMcluster[min_rmsum_cluster1]
        if window_counter1 % 10 == 0:
            if ctr1 == 7:
                print 'SATISFIED'
                flag1 = True
                try:
                    a1rejects[min_rmsum_cluster1].append(track_remove_1)
                except KeyError:
                    a1rejects[min_rmsum_cluster1] =[track_remove_1]
            ctr1 = 0
        prev1 = curr1
        ...
        window_counter2 = window_counter2 + 1
        #print RMcluster
        min_rmsum_cluster2 = min(RMcluster, key=RMcluster.get)
        #print agent2_dict_temp[min_rmsum_cluster2]
        track_remove_2 = min(track_merits2.viewkeys() & agent2_dict_temp[min_rmsum_cluster2], key=track_merits2.get)
        curr2 = RMcluster[min_rmsum_cluster2]
        if prev2>=curr2:
            ctr2 = ctr2+1
        print 'a2','\t',it,'\t',prev2,'\t',curr2,'\t',flag2,'\t',window_counter2,'\t',ctr2,'\t',min_rmsum_cluster2,'\t',RMcluster[min_rmsum_cluster2]
        if window_counter2 % 10 == 0:
            if ctr2 == 7:
                print 'SATISFIED'
                flag2 = True

                try:
                    a2rejects[min_rmsum_cluster2].append(track_remove_2)
                except KeyError:
                    a2rejects[min_rmsum_cluster2] =[track_remove_2]
            ctr2 = 0
        prev2 = curr2
        try:
