...
def calculate_pair_reward(i,j,agent):
	global agent1_dict, agent2_dict,track_rewards1,track_rewards2,track_knn1,track_knn2,track_a2reward
	value = 0
	intersum = 0
	cluster_knnset = set([])
	cluster_knn1 = {}
	...
	if(agent==1):
		...
		for t in agent1_dict[i]:
			...
			if inter!=set([]):
				for i in inter:
					print 'common member ',i,'\t','reward in a2 ',track_rewards2[i]
					a2rewardsum += track_rewards2[i]
				intersum += a2rewardsum
			else:
				a2rewardsum = 0
			track_a2reward[t] = a2rewardsum
			print 'reward sum in a2 ',a2rewardsum		
		cluster_knn1[i] = cluster_knnset
		print 'knn set for a1 cluster ',cluster_knnset		
		not_in_knnset = set(agent2_dict[j]).difference(cluster_knnset)
		print 'not in knnset ',not_in_knnset
		not_in_knnset_sum = sum(track_rewards2[i] for i in not_in_knnset)
		print 'intersum ',intersum
		value = intersum - (w*not_in_knnset_sum)
		print 'pair reward ',value 
	else:
...
def main():
	...
	#display()
	a1a2reward_dict = totalPairReward()
